document.addEventListener('DOMContentLoaded', () => {
        ...
        function createBackgroundVideo(src, volume = 0.4) {
  ...
  return video;
        ...
        function showCard(s){ 
            ...
            div.innerHTML=`<div style="position:relative">${freeTag}<img class="thumb lazy-img" src="${PLACEHOLDER_IMG}" data-src="${s.thumbnail||''}" alt="${s.title}"><div class="progress-bar" style="display:none"><span></span></div></div><div class="meta"><div class="name">${s.title}</div></div>`; 
            div.onclick = () => {
                openShowModal(s);
        ...
        function closeShow(){ document.querySelectorAll('.bg-video').forEach(v => {
  v.pause();
  v.remove();
});

showModal.classList.remove('open'); // Remove ?show from URL
  const url = new URL(window.location);
  ...
  window.history.pushState({}, "", url); document.body.classList.remove('no-scroll'); }
        ...
        function openShowModal(show){
          try{
            ...
            const oldVideo = banner.querySelector('.bg-video');
const oldMute = banner.querySelector('.video-mute-btn');
if (oldVideo) oldVideo.remove();
if (oldMute) oldMute.remove();

if (show.bannerVideo) {
  const video = createBackgroundVideo(show.bannerVideo);
...
function renderHero(items) {
  ...
  // rest of renderHero continues...
          ...
          function show(i){ idx=(i+items.length)%items.length; slides().forEach((el,j)=>el.classList.toggle('active',j===idx));if (el._bgVideo) {
  j === idx ? el._bgVideo.play().catch(()=>{}) : el._bgVideo.pause();
}
