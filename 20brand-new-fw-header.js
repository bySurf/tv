(function() {
    ...
    function isHomePageOrJoin() {
      if (typeof window === 'undefined' || !window.location) return false;
      var path = (window.location.pathname || '/').replace(/\/$/, '') || '/';
      return path === '/' || path === '/join' || path === '/pages/join' || path.indexOf('/pages/join') !== -1;
    }

    /** Re-check auth (and cache) only when user visits bysurf.tv root or supporters profile edit. */
    function isAuthRefreshPage() {
      if (typeof window === 'undefined' || !window.location) return false;
      var path = (window.location.pathname || '/').replace(/\/$/, '') || '/';
      if (path === '' || path === '/') return true;
      if (path.indexOf('/supporters/users/supporters/profile/edit') !== -1) return true;
      return false;
    }
    ...
    function injectNavAndScript() {
      ...
      style.textContent = [
        ...
        '.mobile-bottom-nav { position: fixed; bottom: 0; left: 0; right: 0; height: 72px; display: none; background: var(--bg); border-top: 2px solid rgba(255,255,255,0.08); box-shadow: 0px 0px 38px #009dff40; z-index: 9999; align-items: flex-end; padding-bottom: 8px; }',
        '@media (max-width: 768px) { .mobile-bottom-nav { display: flex; justify-content: space-around; align-items: flex-end; } .sticky-nav { display: none; } }',
        '.nav-btn { position: relative; flex: 1; background: none; border: none; color: #fff; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 6px 0 4px; gap: 2px; transition: color 0.2s ease; cursor: pointer; text-decoration: none; }',
        '.nav-btn-icon { display: flex; align-items: center; justify-content: center; } .nav-btn-label { font-size: 0.65rem; font-weight: 700; color: var(--muted); line-height: 1.1; }',
        '.nav-btn.active .nav-btn-label, .nav-btn:active .nav-btn-label { color: var(--accent); }',
        '.nav-btn svg { width: 24px; height: 24px; fill: currentColor; } .nav-btn .discord-icon { width: 24px; height: 24px; }',
        ...
        '.mobile-nav-search-slot { flex: 1; max-width: 56px; width: 56px; height: 56px; margin-top: -20px; background: var(--bg); border-radius: 50%; display: flex; align-items: center; justify-content: center; }',
        '.mobile-nav-search-slot a { display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; color: var(--muted); } .mobile-nav-search-slot a:hover { color: #fff; }',
        'body.no-scroll { overflow: hidden; }',
        '.fw-profile-dropdown-wrap { position: relative; }',
        '.fw-profile-dropdown-wrap.fw-profile-dropdown-disabled .fw-profile-dropdown { display: none !important; }',
      ...
      var exclamationSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle" viewBox="0 0 16 16"><path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/><path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0M7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0z"/></svg>';
      var exclamationFillSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle-fill" viewBox="0 0 16 16"><path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M8 4a.905.905 0 0 0-.9.995l.35 3.507a.552.552 0 0 0 1.1 0l.35-3.507A.905.905 0 0 0 8 4m.002 6a1 1 0 1 0 0 2 1 1 0 0 0 0-2"/></svg>';
      var mobileNavHtml = '<nav class="mobile-bottom-nav" id="mobileBottomNav">' +
        '<button class="nav-btn nav-btn-home" data-action="home" aria-label="Home"><span class="nav-btn-icon">' + homeOutline + '</span><span class="nav-btn-label">Home</span></button>' +
        '<a href="' + BASE + '/join" class="nav-btn nav-btn-join hidden" id="mobileJoinBtn" aria-label="Join"><span class="nav-btn-icon">' + starSvg + '</span><span class="nav-btn-label">Join</span></a>' +
        '<a href="' + BASE + '/join" class="nav-btn nav-btn-upgrade hidden" id="mobileUpgradeBtn" aria-label="Upgrade"><span class="nav-btn-icon">' + awardSvg + '</span><span class="nav-btn-label">Upgrade</span></a>' +
        '<a href="' + BASE + '/supporters/perks" class="nav-btn nav-btn-lounge hidden" id="mobileLoungeBtn" aria-label="Lounge" target="_blank" rel="noopener"><span class="nav-btn-icon">' + discordSvg + '</span><span class="nav-btn-label">Lounge</span></a>' +
        '<a href="' + BASE + '/pages/report-bugs" class="nav-btn nav-btn-bugs" id="mobileBugsBtn" aria-label="Bugs"><span class="nav-btn-icon">' + exclamationSvg + '</span><span class="nav-btn-label">Bugs</span></a>' +
        '<button class="nav-btn hidden" data-action="rewards" aria-label="Rewards" id="mobileRewardsBtn"><span class="nav-btn-icon">' + giftOutline + '</span><span class="nav-btn-label">Rewards</span></button>' +
        '<button class="nav-btn" data-action="account" aria-label="Account" id="mobileAccountBtn"><span class="nav-btn-icon"><div class="pfp-wrap"></div></span><span class="nav-btn-label">Account</span></button>' +
        '</nav>';
      ...
      var stickyNavHtml = '<header class="sticky-nav" id="fwStickyNav">' +
        '<div class="nav-container">' +
          ...
          '<div class="nav-right">' +
            ...
            '<a href="' + BASE + '/join" class="nav-button accent" id="join-button">Join</a>' +
          '</div>' +
    ...
    function runNavScript() {
      ...
      var giftFillSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-gift-fill" viewBox="0 0 16 16"><path d="M3 2.5a2.5 2.5 0 0 1 5 0 2.5 2.5 0 0 1 5 0v.006c0 .07 0 .27-.038.494H15a1 1 0 0 1 1 1v1a1 1 0 0 1-1 1H1a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h2.038A3 3 0 0 1 3 2.506zm1.068.5H7v-.5a1.5 1.5 0 1 0-3 0c0 .085.002.274.045.43zM9 3h2.932l.023-.07c.043-.156.045-.345.045-.43a1.5 1.5 0 0 0-3 0zm6 4v7.5a1.5 1.5 0 0 1-1.5 1.5H9V7zM2.5 16A1.5 1.5 0 0 1 1 14.5V7h6v9z"/></svg>';
      var homeFillSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16"><path d="M6.5 14.5v-3.505c0-.245.25-.495.5-.495h2c.25 0 .5.25.5.5v3.5a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 .5-.5v-7a.5.5 0 0 0-.146-.354L13 5.793V2.5a.5.5 0 0 0-.5-.5h-1a.5.5 0 0 0-.5.5v1.293L8.354 1.146a.5.5 0 0 0-.708 0l-6 6A.5.5 0 0 0 1.5 7.5v7a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 .5-.5"/></svg>';
      var homeOutlineSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-house-door" viewBox="0 0 16 16"><path d="M8.354 1.146a.5.5 0 0 0-.708 0l-6 6A.5.5 0 0 0 1.5 7.5v7a.5.5 0 0 0 .5.5h4.5a.5.5 0 0 0 .5-.5v-4h2v4a.5.5 0 0 0 .5.5H14a.5.5 0 0 0 .5-.5v-7a.5.5 0 0 0-.146-.354L13 5.793V2.5a.5.5 0 0 0-.5-.5h-1a.5.5 0 0 0-.5.5v1.293zM2.5 14V7.707l5.5-5.5 5.5 5.5V14H10v-4a.5.5 0 0 0-.5-.5h-3a.5.5 0 0 0-.5.5v4z"/></svg>';
      var giftOutlineSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-gift" viewBox="0 0 16 16"><path d="M3 2.5a2.5 2.5 0 0 1 5 0 2.5 2.5 0 0 1 5 0v.006c0 .07 0 .27-.038.494H15a1 1 0 0 1 1 1v2a1 1 0 0 1-1 1v7.5a1.5 1.5 0 0 1-1.5 1.5h-11A1.5 1.5 0 0 1 1 14.5V7a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h2.038A3 3 0 0 1 3 2.506zm1.068.5H7v-.5a1.5 1.5 0 1 0-3 0c0 .085.002.274.045.43zM9 3h2.932l.023-.07c.043-.156.045-.345.045-.43a1.5 1.5 0 0 0-3 0zM1 4v2h6V4zm8 0v2h6V4zm5 3H9v8h4.5a.5.5 0 0 0 .5-.5zm-7 8V7H2v7.5a.5.5 0 0 0 .5.5z"/></svg>';
      var exclamationSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle" viewBox="0 0 16 16"><path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/><path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0M7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0z"/></svg>';
      var exclamationFillSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle-fill" viewBox="0 0 16 16"><path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M8 4a.905.905 0 0 0-.9.995l.35 3.507a.552.552 0 0 0 1.1 0l.35-3.507A.905.905 0 0 0 8 4m.002 6a1 1 0 1 0 0 2 1 1 0 0 0 0-2"/></svg>';

      function applyMobileNavActiveState() {
        var path = (window.location.pathname || '/').replace(/\/$/, '') || '/';
        var isJoinPage = path === '/join' || path === '/pages/join' || path.indexOf('/pages/join') !== -1;
        var nav = document.getElementById('mobileBottomNav');
        if (!nav) return;
        var homeBtn = nav.querySelector('.nav-btn-home, [data-action="home"]');
        var rewardsBtn = document.getElementById('mobileRewardsBtn');
        var accountBtn = document.getElementById('mobileAccountBtn');
        nav.classList.remove('account-settings-active');
        if (homeBtn) {
          homeBtn.classList.remove('home-page-active');
          var icon = homeBtn.querySelector('.nav-btn-icon');
          if (icon) icon.innerHTML = (path === '/' || path === '/videos') ? homeFillSvg : homeOutlineSvg;
          if (path === '/' || path === '/videos') homeBtn.classList.add('home-page-active');
        }
        var joinBtn = document.getElementById('mobileJoinBtn');
        var upgradeBtn = document.getElementById('mobileUpgradeBtn');
        if (joinBtn && !joinBtn.classList.contains('hidden')) {
          joinBtn.classList.remove('join-page-active');
          var jIcon = joinBtn.querySelector('.nav-btn-icon');
          if (jIcon) jIcon.innerHTML = isJoinPage ? starFillSvg : starSvg;
          if (isJoinPage) joinBtn.classList.add('join-page-active');
        }
        if (upgradeBtn && !upgradeBtn.classList.contains('hidden')) {
          upgradeBtn.classList.remove('join-page-active');
          var uIcon = upgradeBtn.querySelector('.nav-btn-icon');
          if (uIcon) uIcon.innerHTML = isJoinPage ? awardFillSvg : awardSvg;
          if (isJoinPage) upgradeBtn.classList.add('join-page-active');
        }
        if (rewardsBtn) {
          rewardsBtn.classList.remove('rewards-page-active');
          var rIcon = rewardsBtn.querySelector('.nav-btn-icon');
          if (rIcon) {
            rIcon.innerHTML = (path === '/rewards' || path === '/pages/rewards') ? giftFillSvg : giftOutlineSvg;
            if (path === '/rewards' || path === '/pages/rewards') rewardsBtn.classList.add('rewards-page-active');
          }
        }
        var bugsBtn = document.getElementById('mobileBugsBtn');
        var isBugsPage = path === '/report-bugs' || path === '/pages/report-bugs' || path.indexOf('/pages/report-bugs') !== -1;
        if (bugsBtn) {
          bugsBtn.classList.remove('bugs-page-active');
          var bIcon = bugsBtn.querySelector('.nav-btn-icon');
          if (bIcon) bIcon.innerHTML = isBugsPage ? exclamationFillSvg : exclamationSvg;
          if (isBugsPage) bugsBtn.classList.add('bugs-page-active');
        }
        if (path.indexOf('/supporters/users/supporters/profile/edit') !== -1 || path.indexOf('/supporters/perks') !== -1 || path.indexOf('/supporters/badges') !== -1 || path.indexOf('/supporters/payments/billing') !== -1) {
          nav.classList.add('account-settings-active');
        }
      }
  
      var AUTH_CACHE_KEY = 'fw-nav-auth';
      ...
      function getCachedAuth() {
        try {
          ...
          if (typeof parsed.isLoggedIn !== 'boolean' || typeof parsed.isPaidMember !== 'boolean') return null;
          return { isLoggedIn: parsed.isLoggedIn, isPaidMember: parsed.isPaidMember };
        } catch (e) {
          return null;
        }
      }

      function setCachedAuth(isLoggedIn, isPaidMember) {
        try {
          sessionStorage.setItem(AUTH_CACHE_KEY, JSON.stringify({ isLoggedIn: isLoggedIn, isPaidMember: isPaidMember }));
        } catch (e) {}
      }

      async function getAuthState() {
        try {
          var signInRes = await fetch(BASE + '/supporters/sign_in', { credentials: 'include' });
          var isLoggedIn = signInRes.redirected;
          if (isLoggedIn) {
            var perksRes = await fetch(BASE + '/supporters/perks', { credentials: 'include' });
            if (perksRes.ok) {
              var html = await perksRes.text();
              IS_PAID_MEMBER = html && html.indexOf('Your perks') !== -1;
            } else {
              IS_PAID_MEMBER = false;
            }
            setCachedAuth(true, IS_PAID_MEMBER);
            return true;
          }
          IS_PAID_MEMBER = false;
          setCachedAuth(false, false);
          return false;
        } catch (e) {
          IS_PAID_MEMBER = false;
          setCachedAuth(false, false);
      ...
      function setupMobileNav(isLoggedIn) {
        ...
        nav.addEventListener('click', function(e) {
          ...
          if (action === 'account') {
            if (isLoggedIn && sheet && backdrop && !isHomePageOrJoin()) {
              sheet.classList.add('open');
              backdrop.classList.add('open');
              document.body.classList.add('no-scroll');
            } else if (isLoggedIn && isHomePageOrJoin()) {
              window.location.href = BASE + '/supporters/users/supporters/profile/edit';
            } else {
        ...
        var mobileJoin = document.getElementById('mobileJoinBtn');
        var mobileUpgrade = document.getElementById('mobileUpgradeBtn');
        var mobileLounge = document.getElementById('mobileLoungeBtn');
        if (mobileJoin) mobileJoin.classList.add('hidden');
        if (mobileUpgrade) mobileUpgrade.classList.add('hidden');
        if (mobileLounge) mobileLounge.classList.add('hidden');
        if (!isLoggedIn) {
          if (mobileJoin) mobileJoin.classList.remove('hidden');
        } else if (!IS_PAID_MEMBER) {
        ...
        if (mobileBugsBtn) {
          if (isLoggedIn) mobileBugsBtn.classList.add('hidden');
          else mobileBugsBtn.classList.remove('hidden');
        }
        if (mobileJoin) {
          var jIcon = mobileJoin.querySelector('.nav-btn-icon');
          if (jIcon) {
            mobileJoin.addEventListener('pointerdown', function() { jIcon.innerHTML = starFillSvg; mobileJoin.classList.add('active'); });
            mobileJoin.addEventListener('pointerup', function() { jIcon.innerHTML = starSvg; mobileJoin.classList.remove('active'); });
            mobileJoin.addEventListener('pointerleave', function() { jIcon.innerHTML = starSvg; mobileJoin.classList.remove('active'); });
          }
        }
        if (mobileUpgrade) {
          var uIcon = mobileUpgrade.querySelector('.nav-btn-icon');
          if (uIcon) {
            mobileUpgrade.addEventListener('pointerdown', function() { uIcon.innerHTML = awardFillSvg; mobileUpgrade.classList.add('active'); });
            mobileUpgrade.addEventListener('pointerup', function() { uIcon.innerHTML = awardSvg; mobileUpgrade.classList.remove('active'); });
            mobileUpgrade.addEventListener('pointerleave', function() { uIcon.innerHTML = awardSvg; mobileUpgrade.classList.remove('active'); });
          }
        }
        var mobileBugs = document.getElementById('mobileBugsBtn');
        if (mobileBugs) {
          var bIcon = mobileBugs.querySelector('.nav-btn-icon');
          if (bIcon) {
            mobileBugs.addEventListener('pointerdown', function() { bIcon.innerHTML = exclamationFillSvg; mobileBugs.classList.add('active'); });
            mobileBugs.addEventListener('pointerup', function() { bIcon.innerHTML = exclamationSvg; mobileBugs.classList.remove('active'); });
            mobileBugs.addEventListener('pointerleave', function() { bIcon.innerHTML = exclamationSvg; mobileBugs.classList.remove('active'); });
          }
        }
      }
      ...
      function updateNav(isLoggedIn) {
        ...
        if (isLoggedIn) {
          ...
          var dropdownWrap = document.querySelector('.fw-profile-dropdown-wrap');
          if (dropdownWrap) {
            if (isHomePageOrJoin()) dropdownWrap.classList.add('fw-profile-dropdown-disabled');
            else dropdownWrap.classList.remove('fw-profile-dropdown-disabled');
          }
          if (IS_PAID_MEMBER) {
      ...
      function initProfileDropdown() {
        ...
        trigger.addEventListener('click', function(e) {
          e.preventDefault();
          e.stopPropagation();
          if (wrap && wrap.classList.contains('fw-profile-dropdown-disabled')) {
            window.location.href = BASE + '/supporters/users/supporters/profile/edit';
            return;
          }
      ...
      function init() {
        ...
        var shouldRefresh = isAuthRefreshPage();
        if (!shouldRefresh && cached) {
          IS_PAID_MEMBER = cached.isPaidMember;
          updateNav(cached.isLoggedIn);
          applyMobileNavActiveState();
        } else {
        ...
        window.addEventListener('focus', function() {
          if (document.getElementById('mobileBottomNav')) applyMobileNavActiveState();
        });
      }
      window.__fwApplyMobileNavActiveState = applyMobileNavActiveState;
    ...
    document.addEventListener('turbo:load', function() {
      ...
      if (isNavOverridePage() && typeof window.__fwApplyMobileNavActiveState === 'function') {
        window.__fwApplyMobileNavActiveState();
      }
    });
    document.addEventListener('turbo:render', function() {
      if (isNavOverridePage()) {
        runOverride();
        if (typeof window.__fwApplyMobileNavActiveState === 'function') window.__fwApplyMobileNavActiveState();
      }
    });
