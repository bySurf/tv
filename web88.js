...
const EP_DURATION_CACHE = new Map();

async function fetchEpisodeDuration(video) {
  if (!video?.url) return null;
  if (video.duration) return video.duration;
  if (EP_DURATION_CACHE.has(video.url)) return EP_DURATION_CACHE.get(video.url);

  try {
    const res = await fetch(video.url, { credentials: 'include' });
    const html = await res.text();
    const doc = new DOMParser().parseFromString(html, 'text/html');

    const lockedInfo = doc.querySelector('.post-locked__info');
    if (lockedInfo) {
      const duration = lockedInfo.textContent.trim();
      EP_DURATION_CACHE.set(video.url, duration);
      video.duration = duration;
      return duration;
    }

    EP_DURATION_CACHE.set(video.url, 'PLAYER');
    return null;

  } catch (err) {
    console.warn('Duration fetch failed:', video.url);
    EP_DURATION_CACHE.set(video.url, null);
    return null;
  }
}
...
function openIframe(v) {
  ...
  isModalOpen = true;
  wrap.innerHTML = `
    <iframe src="${v.url}" class="player-frame"
  ...
  iframe.addEventListener('load', () => {
    try {
      ...
      const videoEl = muxPlayer.querySelector('video');
      if (!videoEl) return;
      ...
      videoEl.addEventListener('loadedmetadata', () => {
        ...
        EP_DURATION_CACHE.set(v.url, formatted);

        document
          .querySelectorAll(`.ep-duration[data-url="${v.url}"]`)
  ...
  list.forEach(v => {
    ...
    el.innerHTML = `
      ...
      <div>
        ...
        <div class="d ep-duration" data-url="${v.url}">
        ⏱ …
        </div>
      </div>
    `;
  ...
  epsEl.querySelectorAll('.ep-duration').forEach(async el => {
    const url = el.dataset.url;
    const video = list.find(v => v.url === url);
    if (!video) return;

    const duration = await fetchEpisodeDuration(video);
    if (duration) {
      ...
      el.textContent = '';
        ...
        function renderHero(items){
          ...
          // Sort items by heroOrder and please stop. just stop brother
