...
async function fetchWatchHistoryHTML() {
  ...
  try {
    ...
    const html = await res.text();
    const payloadA = {location:'fullweb.js:fetchWatchHistoryHTML',message:'History fetch result',data:{ok:res.ok,status:res.status,htmlLen:html.length,hasHistoryList:html.includes('history_posts_list'),hasProgressBar:html.includes('video__progress-bar')},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'A'};
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payloadA)}).catch(()=>{});
    console.log('[BYW]', payloadA);
    if (!res.ok) throw new Error('Failed to fetch history');
    return html;
  } catch (e) {
    const payloadErr = {location:'fullweb.js:fetchWatchHistoryHTML',message:'History fetch error',data:{err:String(e.message)},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'A'};
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payloadErr)}).catch(()=>{});
    console.log('[BYW]', payloadErr);
    throw e;
  }
  // #endregion
}
...
function parseFullyWatchedEpisodes(html) {
  ...
  // #region agent log
  const firstFewPcts = videos.slice(0, 5).map(v => ({ pct: v.querySelector('.video__progress-bar')?.style?.width, title: v.querySelector('.video__title')?.textContent?.trim() }));
  const payloadE = {location:'fullweb.js:parseFullyWatchedEpisodes',message:'Parse progress values',data:{videoCount:videos.length,firstFewPcts},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'E'};
  fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payloadE)}).catch(()=>{});
  console.log('[BYW]', payloadE);
  // #endregion
  ...
  return videos
    .map(video => {
      ...
      const pct = bar?.style?.width?.replace('%', '');

      if (pct === '100.0' && title) return title;
      return null;
    })
    .filter(Boolean);
}
...
if (searchInput) {
  ...
  document.querySelectorAll('.section').forEach(sec => {
    ...
    // Hide ALL other sections including Continue Watching
    document.querySelectorAll('.section').forEach(sec => {
      if (sec.id !== 'searchResults') {
        sec.classList.add('hidden');
      }
    });

    // Find matching shows via episodes + tags
    ...
    (window.__ALL_VIDEOS__ || []).forEach(v => {
      ...
      if (haystack.includes(q)) {
        matchedShows.set(v.show, v.show);
      }
    });
(window.__ALL_SHOWS__ || []).forEach(s => {
  ...
  if (haystack.includes(q)) {
    matchedShows.set(s.title, s.title);
  }
});
    ...
    matchedShows.forEach(showTitle => {
      ...
      if (showObj) {
        searchRail.appendChild(showCard(showObj));
      }
    });
...
const searchBar = searchInput?.closest('.search-wrap');
...
// === Start / Continue Watching button ===
const btn = document.createElement('button');
btn.className = 'start-watching-btn';

const historyMatch = (FW_HISTORY || []).find(
  h => h.show === show.title
);
...
if (historyMatch) {
  btn.innerHTML = `
    ...
    Continue Watching
  `;
  btn.onclick = () => openPlayer(historyMatch);
} else {
  btn.innerHTML = `
    <svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
    Start Watching
  `;
const startPlayback = () => {
  const episodes = (window.__ALL_VIDEOS__ || []).filter(
    v => v.show === show.title && v.url
  );
  if (!episodes.length) return;

  setTimeout(() => {
    openPlayer(episodes[0]);
  }, 140); // allow ripple to render
};
...
window.addEventListener('resize', adjustShowMobileSpacing);
        ...
        function renderHero(items){
          ...
          hero.innerHTML = `<button class="nav prev" aria-label="Previous">❮</button><div id="hero-dots"></div><button class="nav next" aria-label="Next">❯</button>`; // Clear old slides, keep nav
  ...
  s.setAttribute('data-bg', getResizedUrl(v.bgUrl, 1400));
...
heroMuteBtn.onclick = (e) => {
  ...
  dotsWrap.appendChild(d);
          ...
          let idx=0; setTimeout(() => show(0), 1200);
          let startX = 0;
let endX = 0;
...
hero.addEventListener('touchend', () => {
 ...
 const slides=()=>Array.from(document.querySelectorAll('.hero-slide')); const dots=()=>Array.from(document.querySelectorAll('.dot'));
          function show(i){
...
if (!vid.src) {
  ...
  dots().forEach((el,j)=>el.classList.toggle('active',j===idx));
}
...
return { history: finalItems, isLoggedIn: true };
  ...
  try {
    ...
    const watchedTitles = parseFullyWatchedEpisodes(html);

    // #region agent log
    const payloadParse = {location:'fullweb.js:buildBecauseYouWatchedRail',message:'After parse',data:{watchedTitlesLen:watchedTitles.length,watchedTitlesSample:watchedTitles.slice(0,3)},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'A'};
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payloadParse)}).catch(()=>{});
    console.log('[BYW]', payloadParse);
    // #endregion
    if (!watchedTitles.length) return;

    const showTitle = getMostRecentCompletedShow(watchedTitles);
    // #region agent log
    const resolveSamples = watchedTitles.slice(0, 3).map(t => ({ title: t, resolvedShow: resolveShowFromEpisodeTitle(t) }));
    const payloadB = {location:'fullweb.js:buildBecauseYouWatchedRail',message:'Resolve show from titles',data:{showTitle,resolveSamples},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'B'};
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payloadB)}).catch(()=>{});
    console.log('[BYW]', payloadB);
    // #endregion
    if (!showTitle) return;
    ...
    const payloadCD = {location:'fullweb.js:buildBecauseYouWatchedRail',message:'Before insert',data:{recommendationsLen:recommendations.length,sourceShowTitle:sourceShow?.title,newSectionExists:!!newSection},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'C,D'};
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payloadCD)}).catch(()=>{});
    console.log('[BYW]', payloadCD);
    // #endregion
    ...
    // #region agent log
    const payloadInserted = {location:'fullweb.js:buildBecauseYouWatchedRail',message:'Rail inserted',data:{inserted:true},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'D'};
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payloadInserted)}).catch(()=>{});
    console.log('[BYW]', payloadInserted);
    // #endregion
    ...
    // #region agent log
    const payloadErrByw = {location:'fullweb.js:buildBecauseYouWatchedRail',message:'Because-you-watched error',data:{err:String(err?.message||err)},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'A'};
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payloadErrByw)}).catch(()=>{});
    console.log('[BYW]', payloadErrByw);
    // #endregion
...
function isShowCompleted(showTitle, watchedTitles) {
  const episodes = (window.__ALL_VIDEOS__ || []).filter(v => v.show === showTitle);
  if (!episodes.length) return false;

  const watchedForShow = episodes.filter(e =>
    watchedTitles.includes(e.title)
  );
...
function getMostRecentCompletedShow(watchedTitles) {
  for (const title of watchedTitles) {
    const show = resolveShowFromEpisodeTitle(title);
    if (!show) continue;

    if (isShowCompleted(show, watchedTitles)) {
      return show;
    }
  }
  return null;
