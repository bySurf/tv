...
function parseFullyWatchedEpisodes(html) {
  ...
  const payloadE = {location:'fullweb.js:parseFullyWatchedEpisodes',message:'Parse progress values',data:{videoCount:videos.length,itemsLen:items.length,firstFewPcts},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'E'};
...
return { history: finalItems, isLoggedIn: true };
  ...
  try {
    ...
    // #region agent log
    const payloadParse = {location:'fullweb.js:buildBecauseYouWatchedRail',message:'After parse',data:{watchedItemsLen:watchedItems.length,watchedItemsSample:watchedItems.slice(0,3)},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'A'};
    ...
    // #region agent log
    const resolveSamples = watchedItems.slice(0, 3).map(it => ({ videoId: it.videoId, title: it.title, resolvedShow: resolveShowFromVideoId(it.videoId) }));
    const payloadB = {location:'fullweb.js:buildBecauseYouWatchedRail',message:'Resolve show from video IDs',data:{showTitle,resolveSamples},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'B'};
