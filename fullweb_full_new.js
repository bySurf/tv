...
setupHeroVisibilityObserver();
  ...
  return match ? Math.max(0, Math.min(100, match.progressPct || 0)) : 0;
        ...
        /** Returns { video, hasProgress } for the show: "up next" episode and whether user has any watch history for this show. */
        function getUpNextForShow(showTitle) {
          ...
          const showHistory = (FW_HISTORY || []).filter(h => h.show === showTitle);

          const inProgress = showHistory.find(h => {
            const p = h.progressPct || 0;
            return p > 0 && p < 100;
          });
          if (inProgress) {
            const video = (window.__ALL_VIDEOS__ || []).find(v => v.__id === inProgress.id);
            if (video) return { video, hasProgress: true };
          }

          const completed = showHistory.filter(h => (h.progressPct || 0) >= 90);
          let maxIdx = -1;
          completed.forEach(h => {
            const idx = sorted.findIndex(v => v.__id === h.id);
          ...
          return { video: sorted[0], hasProgress: false };
        }
