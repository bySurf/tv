...
const LOCK_ICON_HTML = `
  <div class="lock-tag">
    <svg viewBox="0 0 24 24">
      <path d="M12 1a5 5 0 00-5 5v4H5v14h14V10h-2V6a5 5 0 00-5-5zm-3 9V6a3 3 0 016 0v4H9z"/>
    </svg>
  </div>
`;
...
function videoCard(v){
  ...
  spacer.style.height = overlap > 0 ? `${overlap + 12}px` : '0px';
        ...
        function showCard(s){ 
            ...
            div.innerHTML=`<div style="position:relative">${freeTag}${lockTag}<img class="thumb lazy-img" src="${PLACEHOLDER_IMG}" data-src="${getResizedUrl(s.thumbnail, 600)}" alt="${s.title}"><div class="progress-bar" style="display:none"><span></span></div></div>`; 
            div.onclick = () => {
                openShowModal(s);
...
if (historyMatch) {
  ...
  btn.onclick = () => {
    ...
    if (episodes.length) openPlayer(episodes[0]);
  };
}
