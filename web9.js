...
div.innerHTML=`<div style="position:relative">${freeTag}<img class="thumb lazy-img" src="${PLACEHOLDER_IMG}" data-src="${getResizedUrl(v.thumbnail, 400)}" alt="${v.title}"><div class="progress-bar" style="display:none"><span></span></div></div><div class="meta"><div class="name">${v.title}</div>${sub?`<div class="sub">${sub}</div>`:''}</div>`;
        ...
        function cwCard(v, options={}){
          ...
          div.innerHTML=`<div style="position:relative">
                           <img class="thumb lazy-img" src="${getResizedUrl(v.thumbnail, 400)}" data-src="${getResizedUrl(v.thumbnail, 400)}" alt="${v.title}">
        ...
        function showCard(s){ 
            ...
            div.innerHTML=`<div style="position:relative">${freeTag}<img class="thumb lazy-img" src="${PLACEHOLDER_IMG}" data-src="${getResizedUrl(s.thumbnail, 400)}" alt="${s.title}"><div class="progress-bar" style="display:none"><span></span></div></div>`; 
            div.onclick = () => {
                openShowModal(s);
  ...
  window.history.pushState({}, "", url); document.body.classList.remove('no-scroll'); }
        ...
        function renderHero(items){
          ...
          items.forEach((v,i)=>{
            ...
            s.setAttribute('data-bg', getResizedUrl(v.bgUrl, 1200));
...
videos.forEach(v => {
    if(v.thumbnail) imageUrls.add(getResizedUrl(v.thumbnail, 400));
    if(v.addedToHero && v.bgUrl) imageUrls.add(getResizedUrl(v.bgUrl, 1200));
