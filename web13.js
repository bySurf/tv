...
<script>
    const targetUrl = '/supporters/users/supporters/profile/edit'; 
    ...
    fetch(targetUrl)
        ...
        .then(htmlText => {
            ...
            const linkElement = doc.querySelector('a.sidebar__name');
            if (linkElement) {
                const username = linkElement.textContent.trim();
                const titleToChange = document.querySelector('#cw-section .row-title');

                if (titleToChange) {
                    titleToChange.textContent = "Continue where you left off, " + username;
                }
            }
        })
