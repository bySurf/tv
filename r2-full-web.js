...
const DATA_URL = `https://server.bysurf.tv/bysurf_tv_data.json`;
let FW_HISTORY = [];
let SERVER_HISTORY = [];
...
async function getIsLoggedIn() {
  try {
    const res = await fetch('https://bysurf.tv/supporters/sign_in', { credentials: 'include' });
  ...
  if (v.muted) {
    ...
    iconOff.style.display = 'none';
        ...
        function closePlayer() {
            ...
            isModalOpen = false;
            syncHeroVideoToModals();
            fetchFourthwallHistory().then(({ history, rawHistory }) => {
                SERVER_HISTORY = enrichServerHistory(rawHistory || []);
                FW_HISTORY = history;
                loadCW();
  ...
  return div;
        ...
        function cwCard(v, options={}){
          ...
          removeBtn.addEventListener('click', (e) => {
              ...
              cleared.add(videoId);
              putCleared(cleared);
              FW_HISTORY = (SERVER_HISTORY || []).filter(item => (item.progressPct || 0) < 100 && !cleared.has(item.id));
              loadCW();
          });
          return div;
        }
...
function getHeroFromHistory() {
  const raw = typeof SERVER_HISTORY !== 'undefined' ? SERVER_HISTORY : [];
  const history = raw.filter(h => (h.progressPct || 0) < 100);
...
<div class="hover-gradient"></div>
        <div class="hover-content">
          ${(s.showHasLogo && s.showLogoUrl)
            ? `<div class="hp-logo-wrap"><img class="hp-logo" src="${s.showLogoUrl}" alt="${(s.title || '').replace(/"/g, '&quot;')}"></div>`
...
div.addEventListener('mouseenter', () => {
  hoverDelay = setTimeout(async () => {
    ...
    const edgeThreshold = viewportWidth * 0.25;
    if (rect.left < edgeThreshold) div.style.transformOrigin = 'left center';
    else if (rect.right > viewportWidth - edgeThreshold) div.style.transformOrigin = 'right center';
...
setupHeroVisibilityObserver();
  ...
  const history = typeof SERVER_HISTORY !== 'undefined' && SERVER_HISTORY.length ? SERVER_HISTORY : [];
  if (!history.length || !video.__id) return 0;
  const match = history.find(h => h.id === video.__id);
...
function unwrapCarousel(rail) {
  if (!rail || !rail.parentElement || !rail.parentElement.classList.contains('carousel')) return;
...
function attachCarouselIfOverflowForSection(sectionEl) {
  const rail = sectionEl && sectionEl.querySelector('.rail');
  if (!rail) return;
  if (rail.parentElement && rail.parentElement.classList.contains('carousel')) return;
  requestAnimationFrame(function() {
    ...
    left.className = 'caro-btn left';
    left.innerHTML = '❮';
    const right = document.createElement('button');
    right.className = 'caro-btn right';
    right.innerHTML = '❯';
    const fadeL = document.createElement('div');
    fadeL.className = 'fade left';
    const fadeR = document.createElement('div');
    fadeR.className = 'fade right';
    wrapper.appendChild(left);
    wrapper.appendChild(right);
    wrapper.appendChild(fadeL);
    wrapper.appendChild(fadeR);
    const step = rail.firstElementChild ? rail.firstElementChild.getBoundingClientRect().width : 280;
    left.onclick = function() { rail.scrollBy({ left: -step, behavior: 'smooth' }); };
    right.onclick = function() { rail.scrollBy({ left: step, behavior: 'smooth' }); };
  });
}
...
function runSearch(q) {
  ...
  if (!q) {
    ...
    unwrapCarousel(searchRail);
    document.querySelectorAll('.section').forEach(function(sec) { sec.classList.remove('hidden'); });
    return;
  }
  ...
  requestAnimationFrame(function() {
    attachCarouselIfOverflowForSection(searchResults);
  });
}
...
if (searchToggleBtns.length) {
  ...
  function moveSearchSlotToBody(slot, active) {
    if (!slot || !mobileNav) return;
    if (active) {
      if (slot.parentNode === document.body) return;
      document.body.appendChild(slot);
    } else {
      if (slot.parentNode !== document.body) return;
      var before = mobileBugsBtn || mobileNav.firstElementChild;
      mobileNav.insertBefore(slot, before);
    }
  }
  ...
  searchToggleBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      const slot = searchWrapMobileEl && searchWrapMobileEl.closest('.mobile-nav-search-slot');
      if (bar) bar.classList.toggle('mobile-active');
      const willBeActive = bar && bar.classList.contains('mobile-active');
      if (slot && isMobile) {
        slot.classList.toggle('search-active', willBeActive);
        if (willBeActive) moveSearchSlotToBody(slot, true);
        else moveSearchSlotToBody(slot, false);
      }
  ...
  document.addEventListener('click', (e) => {
    ...
    if (!isMobile) return;
    const slot = searchWrapMobileEl && searchWrapMobileEl.closest('.mobile-nav-search-slot');
    const clickedOutsideSlot = slot && !slot.contains(e.target);
    if (searchToggleBtns.length && !Array.from(searchToggleBtns).some(b => b.contains(e.target)) && clickedOutsideSlot && searchWrapMobileEl && searchWrapMobileEl.classList.contains('mobile-active')) {
      if (slot) {
        moveSearchSlotToBody(slot, false);
        slot.classList.remove('search-active');
      }
      searchWrapMobileEl.classList.remove('mobile-active');
...
hero.addEventListener('touchend', () => {
  ...
  startX = endX = null;
        ...
        function enrichServerHistory(raw) {
          if (!raw || !raw.length) return [];
          const all = window.__ALL_VIDEOS__ || [];
          return raw.map(item => {
            const v = all.find(vid => vid.__id === item.id);
            return { ...item, show: (v && v.show) ? v.show : (item.show || '') };
          });
        }
...
.then(html => {
  ...
  if (homeBtn) {
    homeBtn.classList.add('home-page-active');
    const icon = homeBtn.querySelector('.nav-btn-icon');
    if (icon) icon.innerHTML = houseFillSvg;
  }
  const giftOutlineSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-gift" viewBox="0 0 16 16"><path d="M3 2.5a2.5 2.5 0 0 1 5 0 2.5 2.5 0 0 1 5 0v.006c0 .07 0 .27-.038.494H15a1 1 0 0 1 1 1v2a1 1 0 0 1-1 1v7.5a1.5 1.5 0 0 1-1.5 1.5h-11A1.5 1.5 0 0 1 1 14.5V7a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h2.038A3 3 0 0 1 3 2.506zm1.068.5H7v-.5a1.5 1.5 0 1 0-3 0c0 .085.002.274.045.43zM9 3h2.932l.023-.07c.043-.156.045-.345.045-.43a1.5 1.5 0 0 0-3 0zM1 4v2h6V4zm8 0v2h6V4zm5 3H9v8h4.5a.5.5 0 0 0 .5-.5zm-7 8V7H2v7.5a.5.5 0 0 0 .5.5z"/></svg>';
  const giftFillSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-gift-fill" viewBox="0 0 16 16"><path d="M3 2.5a2.5 2.5 0 0 1 5 0 2.5 2.5 0 0 1 5 0v.006c0 .07 0 .27-.038.494H15a1 1 0 0 1 1 1v1a1 1 0 0 1-1 1H1a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h2.038A3 3 0 0 1 3 2.506zm1.068.5H7v-.5a1.5 1.5 0 1 0-3 0c0 .085.002.274.045.43zM9 3h2.932l.023-.07c.043-.156.045-.345.045-.43a1.5 1.5 0 0 0-3 0zm6 4v7.5a1.5 1.5 0 0 1-1.5 1.5H9V7zM2.5 16A1.5 1.5 0 0 1 1 14.5V7h6v9z"/></svg>';
  const rewardsBtn = document.getElementById('mobileRewardsBtn');
  if (rewardsBtn) {
    const rIcon = rewardsBtn.querySelector('.nav-btn-icon');
    if (rIcon) {
      rewardsBtn.addEventListener('pointerdown', function() {
        rIcon.innerHTML = giftFillSvg;
        rewardsBtn.classList.add('active');
      });
      rewardsBtn.addEventListener('pointerup', function() { rIcon.innerHTML = giftOutlineSvg; rewardsBtn.classList.remove('active'); });
      rewardsBtn.addEventListener('pointerleave', function() { rIcon.innerHTML = giftOutlineSvg; rewardsBtn.classList.remove('active'); });
    }
  }

  const joinBtn = document.getElementById('mobileJoinBtn');
  const upgradeBtn = document.getElementById('mobileUpgradeBtn');
  const loungeBtn = document.getElementById('mobileLoungeBtn');
  if (joinBtn) joinBtn.classList.add('hidden');
  if (upgradeBtn) upgradeBtn.classList.add('hidden');
  if (loungeBtn) loungeBtn.classList.add('hidden');
  if (!isLoggedIn) {
    if (joinBtn) joinBtn.classList.remove('hidden');
  } else if (!IS_PAID_MEMBER) {
    if (upgradeBtn) upgradeBtn.classList.remove('hidden');
  } else {
    if (loungeBtn) loungeBtn.classList.remove('hidden');
  }

  const bugsBtnEl = document.getElementById('mobileBugsBtn');
  if (bugsBtnEl) {
    if (isLoggedIn) bugsBtnEl.classList.add('hidden');
    else bugsBtnEl.classList.remove('hidden');
  }

  const starSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-star" viewBox="0 0 16 16"><path d="M2.866 14.85c-.078.444.36.791.746.593l4.39-2.256 4.389 2.256c.386.198.824-.149.746-.592l-.83-4.73 3.522-3.356c.33-.314.16-.888-.282-.95l-4.898-.696L8.465.792a.513.513 0 0 0-.927 0L5.354 5.12l-4.898.696c-.441.062-.612.636-.283.95l3.523 3.356-.83 4.73zm4.905-2.767-3.686 1.894.694-3.957a.56.56 0 0 0-.163-.505L1.71 6.745l4.052-.576a.53.53 0 0 0 .393-.288L8 2.223l1.847 3.658a.53.53 0 0 0 .393.288l4.052.575-2.906 2.77a.56.56 0 0 0-.163.506l.694 3.957-3.686-1.894a.5.5 0 0 0-.461 0z"/></svg>';
  const starFillSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-star-fill" viewBox="0 0 16 16"><path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/></svg>';
  const awardSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-award" viewBox="0 0 16 16"><path d="M9.669.864 8 0 6.331.864l-1.858.282-.842 1.68-1.337 1.32L2.6 6l-.306 1.854 1.337 1.32.842 1.68 1.858.282L8 12l1.669-.864 1.858-.282.842-1.68 1.337-1.32L13.4 6l.306-1.854-1.337-1.32-.842-1.68zm1.196 1.193.684 1.365 1.086 1.072L12.387 6l.248 1.506-1.086 1.072-.684 1.365-1.51.229L8 10.874l-1.355-.702-1.51-.229-.684-1.365-1.086-1.072L3.614 6l-.25-1.506 1.087-1.072.684-1.365 1.51-.229L8 1.126l1.356.702z"/><path d="M4 11.794V16l4-1 4 1v-4.206l-2.018.306L8 13.126 6.018 12.1z"/></svg>';
  const awardFillSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-award-fill" viewBox="0 0 16 16"><path d="m8 0 1.669.864 1.858.282.842 1.68 1.337 1.32L13.4 6l.306 1.854-1.337 1.32-.842 1.68-1.858.282L8 12l-1.669-.864-1.858-.282-.842-1.68-1.337-1.32L2.6 6l-.306-1.854 1.337-1.32.842-1.68L6.331.864z"/><path d="M4 11.794V16l4-1 4 1v-4.206l-2.018.306L8 13.126 6.018 12.1z"/></svg>';
  if (joinBtn) {
    const jIcon = joinBtn.querySelector('.nav-btn-icon');
    if (jIcon) {
      joinBtn.addEventListener('pointerdown', function() { jIcon.innerHTML = starFillSvg; joinBtn.classList.add('active'); });
      joinBtn.addEventListener('pointerup', function() { jIcon.innerHTML = starSvg; joinBtn.classList.remove('active'); });
      joinBtn.addEventListener('pointerleave', function() { jIcon.innerHTML = starSvg; joinBtn.classList.remove('active'); });
    }
  }
  if (upgradeBtn) {
    const uIcon = upgradeBtn.querySelector('.nav-btn-icon');
    if (uIcon) {
      upgradeBtn.addEventListener('pointerdown', function() { uIcon.innerHTML = awardFillSvg; upgradeBtn.classList.add('active'); });
      upgradeBtn.addEventListener('pointerup', function() { uIcon.innerHTML = awardSvg; upgradeBtn.classList.remove('active'); });
      upgradeBtn.addEventListener('pointerleave', function() { uIcon.innerHTML = awardSvg; upgradeBtn.classList.remove('active'); });
    }
  }

  const bugsBtn = document.getElementById('mobileBugsBtn');
  const exclamationSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle" viewBox="0 0 16 16"><path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/><path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0M7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0z"/></svg>';
  const exclamationFillSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle-fill" viewBox="0 0 16 16"><path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M8 4a.905.905 0 0 0-.9.995l.35 3.507a.552.552 0 0 0 1.1 0l.35-3.507A.905.905 0 0 0 8 4m.002 6a1 1 0 1 0 0 2 1 1 0 0 0 0-2"/></svg>';
  if (bugsBtn) {
    const bIcon = bugsBtn.querySelector('.nav-btn-icon');
    if (bIcon) {
      bugsBtn.addEventListener('pointerdown', function() { bIcon.innerHTML = exclamationFillSvg; bugsBtn.classList.add('active'); });
      bugsBtn.addEventListener('pointerup', function() { bIcon.innerHTML = exclamationSvg; bugsBtn.classList.remove('active'); });
      bugsBtn.addEventListener('pointerleave', function() { bIcon.innerHTML = exclamationSvg; bugsBtn.classList.remove('active'); });
    }
  }

  const homeBtnForPress = nav.querySelector('[data-action="home"]');
  if (homeBtnForPress) {
    homeBtnForPress.addEventListener('pointerdown', function() { homeBtnForPress.classList.add('active'); });
    homeBtnForPress.addEventListener('pointerup', function() { homeBtnForPress.classList.remove('active'); });
    homeBtnForPress.addEventListener('pointerleave', function() { homeBtnForPress.classList.remove('active'); });
  }
}
            ...
            const discordPerksLink = document.getElementById('discord-perks');
          ...
          try{
            const res = await fetch('/supporters/history', { credentials:'include' });
             if (!res.ok || res.redirected) {
                return { history: [], isLoggedIn: false, rawHistory: [] };
            }
            const html = await res.text();
            const isLoggedIn = html.includes('history_posts_list');
            if (!isLoggedIn) return { history: [], isLoggedIn: false, rawHistory: [] };

            const doc = new DOMParser().parseFromString(html, 'text/html');
            const items = [];
            const seenIds = new Set();
            function addVideoItem(a) {
              ...
              seenIds.add(id);
              const title = a.querySelector('.video__title')?.textContent?.trim() || a.querySelector('[class*="title"]')?.textContent?.trim() || 'Episode';
              const thumb = a.querySelector('.video__image')?.getAttribute('src') || a.querySelector('img')?.getAttribute('src') || '';
              const bar = a.querySelector('.video__progress-bar') || a.querySelector('[class*="progress"]');
              let pct = 0;
              if (bar) {
                const w = (bar.style && bar.style.width) || bar.getAttribute('style') || '';
                const m = (typeof w === 'string' ? w : '').match(/([\d.]+)%/);
                if (m) pct = Math.max(0, Math.min(100, parseFloat(m[1])));
              }
              items.push({ id, title, thumbnail: thumb, url: href, show: '', progressPct: pct });
            }
            doc.querySelectorAll('a.video').forEach(addVideoItem);
            doc.querySelectorAll('a[href*="/videos/"]').forEach(function(a) {
              if (!a.classList.contains('video')) addVideoItem(a);
            });
...
return { history: finalItems, isLoggedIn: true, rawHistory: items };
...
function getMostRecentCompletedShow(watchedItems) {
  ...
  return null;
        ...
        (async function(){
          ...
          const { history, rawHistory } = await fetchFourthwallHistory();
          SERVER_HISTORY = enrichServerHistory(rawHistory || []);
          FW_HISTORY = history;
...
function renderBecauseYouWatchedRail() {
  const serverHistory = typeof SERVER_HISTORY !== 'undefined' ? SERVER_HISTORY : [];
