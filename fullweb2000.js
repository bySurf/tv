...
userPrefersMuted = v.muted;
  // #region agent log
  fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fullweb.js:showVideoMute click',message:'show modal mute clicked',data:{userPrefersMuted},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'H2'})}).catch(()=>{});
  // #endregion
...
div.addEventListener('mouseenter', async () => {
  ...
  video.loop = true;
  // #region agent log
  fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fullweb.js:hover mouseenter',message:'hover preview opening',data:{userPrefersMuted},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'H4'})}).catch(()=>{});
  // #endregion
...
function syncHeroVideoToModals() {
  ...
  if (anyOpen) {
    ...
    heroVideos.forEach(v => {
      if (v === activeVid && activeVid) activeVid.play().catch(() => {});
      else v.pause();
    });
  }
}
...
if (show.showModalHasVideo && show.showModalVideoURL) {
  showBannerVideoTimeout = setTimeout(() => {
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fullweb.js:showModal open muteBtn',message:'show modal showing mute btn',data:{userPrefersMuted,syncMuteIconCalled:false},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'H2'})}).catch(()=>{});
    // #endregion
...
heroMuteBtn.onclick = (e) => {
  ...
  // #region agent log
  fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fullweb.js:heroMuteBtn click',message:'hero mute clicked',data:{userPrefersMuted},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'H2'})}).catch(()=>{});
  // #endregion
