...
function renderBecauseYouWatchedRail() {
  ...
  // find first fully watched episode
  const watched = FW_HISTORY.find(h => h.progressPct >= 100);
  if (!watched) return;

  const episode = window.__ALL_VIDEOS__.find(v => v.__id === watched.id);
  if (!episode || !episode.show) return;

  const show = window.__ALL_SHOWS__.find(s => s.title === episode.show);
  if (!show || !show.tags?.length) return;

  // find similar shows by tag
  const tagSet = new Set(show.tags.map(t => t.toLowerCase()));

  const recommendations = window.__ALL_SHOWS__
    .filter(s => s.title !== show.title)
    .map(s => ({
      show: s,
      score: (s.tags || []).filter(t => tagSet.has(t.toLowerCase())).length
    }))
    .filter(x => x.score > 0)
    .sort((a, b) => b.score - a.score)
    .slice(0, 5)
  ...
  section.innerHTML = `
    <div class="row-head">
      <h2 class="row-title">Because you watched ${show.title}</h2>
    </div>
    <div class="rail"></div>
  `;
