...
async function fetchEpisodeDuration(video) {
  ...
  if (EP_DURATION_CACHE.has(video.url)) {
    return EP_DURATION_CACHE.get(video.url);
  }

  try {
    const res = await fetch(video.url, { credentials: 'include' });
    if (!res.ok) throw new Error('Failed to load episode page');

    const html = await res.text();
    const doc = new DOMParser().parseFromString(html, 'text/html');

    const infoEl = doc.querySelector('.post-locked__info');
    const duration = infoEl?.textContent?.trim() || null;
    ...
    console.warn('Duration fetch failed:', video.url, err);
