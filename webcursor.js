...
async function fetchWatchHistoryHTML() {
  ...
  try {
    ...
    const html = await res.text();
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fullweb.js:fetchWatchHistoryHTML',message:'History fetch result',data:{ok:res.ok,status:res.status,htmlLen:html.length,hasHistoryList:html.includes('history_posts_list'),hasProgressBar:html.includes('video__progress-bar')},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'A'})}).catch(()=>{});
    if (!res.ok) throw new Error('Failed to fetch history');
    return html;
  } catch (e) {
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fullweb.js:fetchWatchHistoryHTML',message:'History fetch error',data:{err:String(e.message)},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'A'})}).catch(()=>{});
    throw e;
  }
  // #endregion
}
...
function parseFullyWatchedEpisodes(html) {
  ...
  fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fullweb.js:parseFullyWatchedEpisodes',message:'Parse progress values',data:{videoCount:videos.length,firstFewPcts},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'E'})}).catch(()=>{});
  // #endregion
...
return { history: finalItems, isLoggedIn: true };
  ...
  try {
    ...
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fullweb.js:buildBecauseYouWatchedRail',message:'After parse',data:{watchedTitlesLen:watchedTitles.length,watchedTitlesSample:watchedTitles.slice(0,3)},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'A'})}).catch(()=>{});
    // #endregion
    ...
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fullweb.js:buildBecauseYouWatchedRail',message:'Resolve show from titles',data:{showTitle,resolveSamples},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'B'})}).catch(()=>{});
    // #endregion
    if (!showTitle) return;
    ...
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fullweb.js:buildBecauseYouWatchedRail',message:'Before insert',data:{recommendationsLen:recommendations.length,sourceShowTitle:sourceShow?.title,newSectionExists:!!newSection},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'C,D'})}).catch(()=>{});
    // #endregion
    ...
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fullweb.js:buildBecauseYouWatchedRail',message:'Rail inserted',data:{inserted:true},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'D'})}).catch(()=>{});
    // #endregion
    ...
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/2da28f94-325e-4ca6-bbe2-875871a31b47',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fullweb.js:buildBecauseYouWatchedRail',message:'Because-you-watched error',data:{err:String(err?.message||err)},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'A'})}).catch(()=>{});
    // #endregion
