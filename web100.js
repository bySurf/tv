...
isPaidMember().then(v => IS_PAID_MEMBER = v);
...
userPrefersMuted = v.muted;
  ...
  const iconOn2 = showVideoMuteBtn.querySelector('.icon-on2');
...
div.innerHTML=`<div style="position:relative">${freeTag}<img class="thumb lazy-img" src="${PLACEHOLDER_IMG}" data-src="${getResizedUrl(v.thumbnail, 600)}" alt="${v.title}"><div class="progress-bar" style="display:none"><span></span></div></div><div class="meta"><div class="name">${v.title}</div>${sub?`<div class="sub">${sub}</div>`:''}</div>`;
  ...
  spacer.style.height = overlap > 0 ? `${overlap + 12}px` : '0px';
        ...
        function showCard(s){ 
            ...
            div.innerHTML=`<div style="position:relative">${freeTag}<img class="thumb lazy-img" src="${PLACEHOLDER_IMG}" data-src="${getResizedUrl(s.thumbnail, 600)}" alt="${s.title}"><div class="progress-bar" style="display:none"><span></span></div></div>`; 
            div.onclick = () => {
                openShowModal(s);
...
startBtn.className = 'start-watching-btn';
startBtn.innerHTML = `
...
// Append directly under description
showDesc.appendChild(startBtn);
...
heroMuteBtn.onclick = (e) => {
  ...
  const iconOn2 = heroMuteBtn.querySelector('.icon-on2');
  const iconOff = heroMuteBtn.querySelector('.icon-off');

  if (heroVideo.muted) {
    if (iconOn2) iconOn2.style.display = 'none';
    if (iconOff) iconOff.style.display = 'block';
  } else {
    if (iconOn2) iconOn2.style.display = 'block';
